...
const createCheckoutSession = async (req, res) => {
  try {
    ...
    const requestedAmount = amount === undefined ? undefined : toFiniteNumber(amount);
    if (amount !== undefined && !Number.isFinite(requestedAmount)) {
      return res.status(400).json({ message: "amount must be a valid number" });
    }

    const payAmount = Number(requestedAmount ?? verification.appointment.consultationFee ?? 0);
...
const payForAppointment = async (req, res) => {
  try {
    ...
    if (typeof method !== "string" || !normalizeString(method)) {
      return res.status(400).json({ message: "method must be a non-empty string" });
    }

    if (req.body.simulateFailure !== undefined && typeof simulateFailure !== "boolean") {
      return res.status(400).json({ message: "simulateFailure must be true or false" });
    }
